// app/routes/users/view.tsx

import { useLoaderData, Link, redirect } from "react-router";
import type { Route } from "./+types/view";
import { api } from "~/shared/utils/api.server";
import { getUser } from "~/shared/utils/auth.server"; // Добавили импорт проверки сессии
...
export async function loader({params, request}: Route.LoaderArgs) {
    ...
    // Безопасно очищаем ID от случайных пробелов или переносов строк
    const cleanId = params.id?.trim();

    if (!cleanId) {
        throw new Response("Неверный идентификатор пользователя", {status: 400});
    }
    ...
    try {
        ...
        // 1. Запрашиваем данные пользователя по очищенному ID
        const userRes = await api.get(`/users/${cleanId}`, {headers});

        // 2. Запрашиваем отзывы изолированно, чтобы они не ломали страницу
        let reviews = [];
        try {
            const reviewsRes = await api.get(`/reviews/user-id/${cleanId}`, {headers});
            reviews = reviewsRes.data;
        } catch (e) {
            console.warn("[Public Profile] Не удалось загрузить отзывы для юзера:", cleanId);
        }
        ...
        // 🔥 Выводим в терминал то, что бэкенд пишет в теле ошибки 400 (там будет точная причина)
        console.error(`[Public Profile Loader] Ошибка бэкенда (Статус ${error.response?.status}):`,
...
export default function PublicProfilePage() {
    ...
    const displayReviews = reviews.filter((r: { clientRating: null; developerRating: null; }) => {
        ...
        <div className="flex-1 bg-slate-50 min-h-screen py-10">
            <div className="max-w-3xl mx-auto px-6">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-8 shadow-sm">
                    ...
                    {/* Хедер профиля */}
                    <div
                        className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-6 mb-8 pb-6 border-b border-slate-100">
                        {user.profilePicture ? (
                            <img
                            ...
                            <div
                                className="w-20 h-20 bg-slate-900 rounded-full flex items-center justify-center text-2xl font-black text-white shadow-inner shrink-0">
                                {firstLetter}
                            </div>
                        )}
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 mt-4">
                                ...
                                <span
                                    className="text-slate-900 font-bold text-xs flex items-center gap-1 bg-amber-50 text-amber-700 px-2 py-0.5 rounded border border-amber-100">
                            ...
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-3">
                                ...
                                {user.companyLink && (
                                    <div className="flex items-center gap-2 text-xs">
                                        ...
                                        <a href={user.companyLink} target="_blank" rel="noreferrer"
                        ...
                        {user.role === "DEVELOPER" && (user.socialGitHub || user.socialLinkedIn || user.socialX) && (
                            <div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    {user.socialGitHub && (
                                        <a href={user.socialGitHub} target="_blank" rel="noreferrer"
                                           className="flex items-center gap-2 p-2.5 border border-slate-200 rounded-lg text-xs font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-colors">
                                            <svg className="w-4 h-4 text-slate-900" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                    ...
                                    {user.socialLinkedIn && (
                                        <a href={user.socialLinkedIn} target="_blank" rel="noreferrer"
                                           className="flex items-center gap-2 p-2.5 border border-slate-200 rounded-lg text-xs font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-colors">
                                    ...
                                    {user.socialX && (
                                        <a href={user.socialX} target="_blank" rel="noreferrer"
                                           className="flex items-center gap-2 p-2.5 border border-slate-200 rounded-lg text-xs font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-colors">
                                            <svg className="w-4 h-4 text-slate-900" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                 strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <div className="pt-8 border-t border-slate-100 mt-8">
                            ...
                            {displayReviews.length === 0 ? (
                                ...
                                <div className="space-y-4">
                                    {displayReviews.map((r: { id: Key | null | undefined; commissionTitle: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined; rating: any; authorRole: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined; date: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined; text: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined; }) => (
